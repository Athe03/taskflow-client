...
export async function assignTask(taskId, userId) {
    const { data, error } = await supabase.from('tasks').update({ assigned_to: userId }).eq('id',taskId).select().single()
    if (error) throw error
    return data
}
...
export async function addComment(taskId, content) {
    const { data: { user } } = await supabase.auth.getUser()
    const { data, error } = await supabase.from('comments').insert({ task_id: taskId, author_id: user.id, content }).select('*, author:profiles(username)').single()
    if (error) throw error
    return data
}